...
SELECT b.Generic, SUM(d.Tot_Spndng_2020) AS D_Spend, SUM(b.Tot_Spndng_2020) AS B_Spend,
	SUM(d.Tot_Spndng_2020) / SUM(b.Tot_Spndng_2020) AS Frac_Spend
FROM part_d d JOIN part_b b
	ON d.Generic = b.Generic
WHERE d.Mftr_Name != 'Overall'
GROUP BY b.Generic
...
SELECT Generic,
	   SUM(Tot_Spndng_2016) AS 2016_Totals,
	   SUM(Tot_Spndng_2017) AS 2017_Totals,
       SUM(Tot_Spndng_2018) AS 2018_Totals,
       SUM(Tot_Spndng_2019) AS 2019_Totals,
	   SUM(Tot_Spndng_2020) AS 2020_Totals
FROM part_d
WHERE Mftr_Name != 'Overall'
GROUP BY Generic
ORDER BY 2020_Totals DESC

SELECT Generic, SUM(Tot_Spndng_2016), SUM(Tot_Spndng_2017), SUM(Tot_Spndng_2018), SUM(Tot_Spndng_2019), SUM(Tot_Spndng_2020)
FROM part_b
WHERE Generic LIKE '%mab'
GROUP BY Generic
ORDER BY Tot_Spndng_2020 DESC, Generic ASC
LIMIT 10